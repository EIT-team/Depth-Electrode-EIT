...
interval = inj_switchT(2:end) - inj_switchT(1:end-1);
...
for iPair =1%:info.Prt_size
    ...
    [Data,HDR] = sread(HDR, round((inj_switch((iPair-1)*info.Prt_rpt+iRpt+1)-inj_switch((iPair-1)*info.Prt_rpt+iRpt))./HDR.SampleRate), inj_switch((iPair-1)*info.Prt_rpt+iRpt)./HDR.SampleRate);
    %[Data,HDR] = sread(HDR, round((inj_switch(iPair+1) - inj_switch(iPair))./HDR.SampleRate), inj_switch(iPair)./HDR.SampleRate);
    Data = Data(:,chns);
    %Find the stim triggers that fall in the timepoint between the
    %injection
    T_trig = find(stim >inj_switch(iPair) & stim < inj_switch(iPair+1));
    T_trigt = stim(T_trig);
    T_trig_inj = T_trigt - inj_switch(iPair); %shift down so they are right time within window
