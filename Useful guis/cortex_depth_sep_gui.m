...
function [S] = cortex_depth_sep_gui(ABS_c, ABS_d, info)
...
S.view1 = uicontrol('Style', 'pushbutton', 'String', 'Clear',...
 ...
 S.view2 = uicontrol('Style', 'pushbutton', 'String', 'Clear',...
                   ...
                   'String', ['Original View'],...
                   'FontSize', 11,'FontWeight','bold',...
                   'Callback', 'S.ax2.View = [90 0]; S.zSlider2.Value = 70; S.ax2.XLim = [S.orig_d(1) - S.zdef_d, S.orig_d(1) + S.zdef_d]; S.ax2.YLim = [S.orig_d(3) - S.zdef_d, S.orig_d(3) + S.zdef_d]; S.ax2.ZLim = [S.orig_d(2) - S.zdef_d, S.orig_d(2) + S.zdef_d];');
...
S.cnts_d = info.cnts_d;
S.ABS_d = ABS_d;      
S.max_d = max(max(ABS_d(:,S.b)));
S.max_val_d = max(max(ABS_d));
S.idx_ABS_d = find(ABS_d(:,S.b) > ((S.a)/100)*S.max_d);

S.cnts_c = info.cnts_c;
S.ABS_c = ABS_c;      
S.max_c = max(max(ABS_c(:,S.b)));
S.max_val_c = max(max(ABS_c));
S.idx_ABS_c = find(ABS_c(:,S.b) > ((S.a)/100)*S.max_c);
...
%S.txttimeSlider.String = ['Time = ' num2str(round(((S.b/3277)*1e3)))];
...
%         S.idx_ABS_d = find(abs(S.ABS_d(:,round(S.b))) > ((S.a)/100)*S.max_val_d); 
%         S.idx_ABS_c = find(abs(S.ABS_c(:,round(S.b))) > ((S.a)/100)*S.max_val_c);
        
          idx1_d = find((S.ABS_d(:,round(S.b))) > ((S.a)/100)*S.max_val_d);
     idx2_d = find((S.ABS_d(:,round(S.b))) < -1*((S.a)/100)*S.max_val_d);
     S.idx_ABS_d = [idx1_d;idx2_d];
     
          idx1_c = find((S.ABS_c(:,round(S.b))) > ((S.a)/100)*S.max_val_c);
      idx2_c = find((S.ABS_c(:,round(S.b))) < -1*((S.a)/100)*S.max_val_c);
     S.idx_ABS_c = [idx1_c;idx2_c];
        
    elseif S.thresPop.Value == 2 && S.negPop.Value == 1
        S.max_d = max(max(S.ABS_d(:,round(S.b))));
        S.idx_ABS_d = find(abs(S.ABS_d(:,round(S.b))) > ((S.a)/100)*S.max_d);
        S.max_c = max(max(S.ABS_c(:,round(S.b))));
        S.idx_ABS_c = find(abs(S.ABS_c(:,round(S.b))) > ((S.a)/100)*S.max_c);
        
    elseif S.thresPop.Value == 1 && S.negPop.Value == 2
        S.idx_ABS_d = find((S.ABS_d(:,round(S.b))) > ((S.a)/100)*S.max_val_d); 
        S.idx_ABS_c = find((S.ABS_c(:,round(S.b))) > ((S.a)/100)*S.max_val_c); 
    else
        S.max_d = max(max(S.ABS_d(:,round(S.b))));
        S.idx_ABS_d = find((S.ABS_d(:,round(S.b))) > ((S.a)/100)*S.max_d);
        S.max_c = max(max(S.ABS_c(:,round(S.b))));
        S.idx_ABS_c = find((S.ABS_c(:,round(S.b))) > ((S.a)/100)*S.max_c);
    end
    
    %plot updated figures
    %First figure
    scatter3(S.ax1, S.cnts_d(S.idx_ABS_d,1),S.cnts_d(S.idx_ABS_d,2),S.cnts_d(S.idx_ABS_d,3), 70, S.ABS_d(S.idx_ABS_d, round(S.b)), 'filled');
    hold on;
    ...
    %Sort out colormap
    colormap(S.ax1,jet);
    S.ax1.CLim = [-round(S.max_val_c),round(S.max_val_c)];
   % cb1 = colorbar(S.ax1, 'Location', 'southoutside', 'Position', [0.09, 0.14, 0.25, 0.03]);
    ...
    %Second figure
    scatter3(S.ax2, S.cnts_d(S.idx_ABS_d,1),-1*S.cnts_d(S.idx_ABS_d,3),-1*S.cnts_d(S.idx_ABS_d,2), 70, S.ABS_d(S.idx_ABS_d, round(S.b)), 'filled');
    hold on;
    ...
    %Sort out colormap
    colormap(S.ax2,jet);
    S.ax2.CLim = [-round(S.max_val_c),round(S.max_val_c)];
    %cb2 = colorbar(S.ax2, 'Location', 'southoutside', 'Position', [0.09, 0.14, 0.25, 0.03]);
    
    
    %Third figure
    scatter3(S.ax3, S.cnts_c(S.idx_ABS_c,1),S.cnts_c(S.idx_ABS_c,2),S.cnts_c(S.idx_ABS_c,3), 70, S.ABS_c(S.idx_ABS_c, round(S.b)), 'filled');
    hold on;
    ...
    %Sort out colormap
    colormap(S.ax3,jet);
    S.ax3.CLim = [-round(S.max_val_c),round(S.max_val_c)];
    ...
    hold on;
    scatter3(S.ax4, S.cnts_d(S.idx_ABS_d,1),S.cnts_d(S.idx_ABS_d,2),S.cnts_d(S.idx_ABS_d,3), 70, S.ABS_d(S.idx_ABS_d, round(S.b)), 'filled');
    patch(S.ax4, 'faces',S.srf_a(S.idx_a,:),'vertices',[S.nodes_a(:,1), S.nodes_a(:,2), S.nodes_a(:,3)],'facevertexcdata',-200*ones(size(S.nodes_a,1),1),...
    'facecolor',[1,1,1],'edgecolor','flat','facelighting', 'none', 'edgelighting', 'flat');  
    hold off;
    daspect(S.ax4,[1,1,1]);
    %Remove ticks
    ...
    %Set zoom to last used zoom
    z = (100-zoom4)/10000;
    S.ax4.XLim = [S.orig_a(1) - z,S.orig_a(1) + z];
    S.ax4.YLim = [S.orig_a(2) - z,S.orig_a(2) + z];
    S.ax4.ZLim = [S.orig_a(3) - z,S.orig_a(3) + z];
%     
%     %Sort out colormap
     colormap(S.ax4,jet);
     S.ax4.CLim = [-round(S.max_val_c),round(S.max_val_c)];
